...
<script>
import {getCaptcha, getSysUserAvatarSrc, sysLogin} from "@/api";
  ...
  watch: {
    'logForm.username': {
      handler() {
        ...
        getSysUserAvatarSrc(params).then(resp => {
          this.$nextTick(f => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              sysLogin(params, this.logForm).then(Response => {
                ...
                this.$store.commit('SET_ISSYSUSER', 1)
                this.$router.push('/sys/bookmanage')
              }, error => {
                this.$message.error(error)
                this.getCaptcha()
              })
